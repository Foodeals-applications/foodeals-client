...
public class RoleController {
    ...
    @GetMapping("/allExcludingAdminAndSuperAdminAndClient")
    public ResponseEntity<List<RoleResponse>> getAllExcludingAdminAndSuperAdminAndClient() {
        final List<RoleResponse> responses = service.findAllExcludingAdminAndSuperAdminAndClient()
                .stream()
                .map(role -> mapper.map(role, RoleResponse.class))
                .toList();
        return ResponseEntity.ok(responses);
