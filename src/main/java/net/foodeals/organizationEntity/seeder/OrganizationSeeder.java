...
// On prend quelques produits existants du seeder
    ...
    private Order createOrder(SubEntity subEntity, Offer offer, Product product, OrderStatus orderStatus) {
        ...
        order.setQuantity(1);
        order.setShippingAddress(offer.getSubEntity().getAddress());
        order.setOrderSource(OrderSource.DEAL_PRO);

        // === Nouveau bloc : calcul des totaux ===
        double salePrice = offer.getSalePrice().amount().doubleValue();
        double deliveryFee = offer.getDeliveryFee() != null ? offer.getDeliveryFee() : 0.0;
        double subtotal = salePrice;
        double tax = subtotal * 0.1; // exemple : 10 % de taxe
        double discount = subtotal * 0.05; // exemple : 5 % de remise
