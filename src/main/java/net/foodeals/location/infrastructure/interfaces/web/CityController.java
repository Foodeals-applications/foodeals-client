...
public class CityController {
   ...
   /* @GetMapping
    ...
    @GetMapping("/all")
    public ResponseEntity<List<CityResponse>> getAll() {
        final List<CityResponse> responses = service.findAll()
                .stream()
                .map(city -> mapper.map(city, CityResponse.class))
                .toList();
        return ResponseEntity.ok(responses);
    ...
    @GetMapping()
    public ResponseEntity<PositionCityResponse> getByName(@RequestParam("name") String name) {
        final City city =service.findByName(name);
        final PositionCityResponse response =new PositionCityResponse(city.getId(), city.getCoordinates());
        return ResponseEntity.ok(response);
    }
